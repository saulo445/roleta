...
<head>
    ...
    <title>Sorteio do Nosso Rolê</title>
    ...
    <style>
        * {
        ...
        .wheel {
            ...
            box-shadow: 0 0 0 5px #ff6b6b, 0 0 20px rgba(255, 107, 107, 0.5);
        }
        ...
        /* NOVOS ESTILOS PARA A IMAGEM AMPLIADA */
        .place-detail {
            display: none;
            margin-top: 30px;
        ...
        .map-button {
            background: linear-gradient(135deg, #4285F4 0%, #2869e6 100%);
            border: none;
            border-radius: 50px;
            padding: 15px 30px;
            color: white;
...
<body>
    <div class="container">
        <h1>🎯 Sorteio do Nosso Rolê</h1>
        
        <div class="instructions">
            <p>Clique no botão "Sortear" e deixe o destino decidir nosso programa!</p>
            <p>Se não gostar do resultado, pode sortear novamente 😉</p>
        </div>
        ...
        <div class="controls">
            <button class="spin-button" onclick="startRoulette()">
                <i class="fas fa-dice"></i> Sortear Agora!
            </button>
        </div>
        
        <div id="result" class="result">
            <p>O destino escolheu: <span id="selected-place"></span>!</p>
            <p>Vamos nesse?</p>
            <div class="timer" id="timer">Confirmando em: 10 segundos</div>
        </div>
        
        <!-- NOVA SEÇÃO PARA DETALHES DO LOCAL -->
        <div id="place-detail" class="place-detail">
            <h2>Detalhes do Local</h2>
            <img id="place-image-large" class="place-image-large" src="" alt="">
            <p>Pronto para nosso encontro no <span id="detail-place-name"></span>?</p>
            <a id="map-link" class="map-button" href="#" target="_blank">
    ...
    <footer>
        Feito especialmente para nosso encontro 💫
    </footer>
    ...
    <script>
        ...
        function startRoulette() {
            ...
            // Sortear um lugar aleatório
            const randomIndex = Math.floor(Math.random() * places.length);
            selectedPlace = places[randomIndex];
            
            // Calcular ângulo para parar no lugar selecionado
            const wheel = document.getElementById('wheel');
            const currentRotation = parseInt(wheel.style.transform.replace('rotate(', '').replace('deg)', '')) || 0;
            const targetAngle = 3600 + (randomIndex * (360 / places.length)); // 10 voltas + posição do item
